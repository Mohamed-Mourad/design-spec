...
export const PUSH_RESPONSE = {
  pull_request_url: 'https://github.com/acme/storefront/pull/43',
...
export async function mockApi(page: Page, opts: MockOptions = {}) {
  ...
  await page.route('**/api/v1/github/push', (route) =>
    route.fulfill({ status: opts.pushStatus ?? 201, json: opts.pushBody ?? PUSH_RESPONSE }),
  )
...
export async function scanStorefront(page: Page) {
